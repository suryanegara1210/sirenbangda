...
<script type="text/javascript">	
	...
	$(document).ready(function(){
		$(".tbh_program").click(function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("rka_perubahan/cru_program_skpd"); ?>',
		...
		$(".remove-program").click(function(){			
			if (confirm('Apakah anda yakin untuk menghapus data program ini?')) {				
				...
				$.ajax({
					type: "POST",
					url: '<?php echo site_url("rka_perubahan/delete_program"); ?>',
		...
		$("#program td.td-click").click(function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("rka_perubahan/get_kegiatan_skpd"); ?>',
				data: {id: idr},				
				success: function(msg){
					if (msg!="") {						
		...
		$(document).on("click", "#get_renja", function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("rka_perubahan/get_renja"); ?>',
				dataType: "json",
				success: function(msg){
	...
	function reload_jendela_kontrol(){
		$("#jendela_kontrol").load("<?php echo site_url('rka_perubahan/get_jendela_kontrol'); ?>");
	}
</script>
...
<article class="module width_full">
	<header>
 		<h3>
			RKA PERUBAHAN <?php echo $nama_skpd; ?>
		</h3>			
 	</header>
			...
			<?php
				if (!empty($program)) {					
					...
					foreach ($program as $row) {
						$i++;
						$style='';
						$indikator_program = $this->m_rka_perubahan->get_indikator_prog_keg_perubahan($row->id);
						$id= $row->id;
