...
<script type="text/javascript">	
	$(document).ready(function(){
		$(".tbh_kegiatan").click(function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("cik/cru_kegiatan_skpd"); ?>',
				data: {id_program: idr},
		...
		$(".remove-kegiatan").click(function(){			
			if (confirm('Apakah anda yakin untuk menghapus data kegiatan ini?')) {
				...
				$.ajax({
					type: "POST",
					url: '<?php echo site_url("cik/delete_kegiatan"); ?>',
		...
		$("#kegiatan td.td-click").click(function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("cik/preview_kegiatan_cik"); ?>',
	...
	<?php
		if (!empty($kegiatan)) {					
			...
			foreach ($kegiatan as $row) {
				$i++;
				$style='';
				$indikator_keg = $this->m_cik->get_indikator_prog_keg($row->id);
				$id= $row->id;
				$parent = $row->parent;
				if($row->id_dpa != NULL){
