...
class M_cik_perubahan extends CI_Model
{
	...
	function count_jendela_kontrol($id_skpd,$ta){
		$query = "SELECT 
					...
					WHERE
						tahun = ?
					AND
						tx_cik_prog_keg_perubahan.id_skpd=? AND is_prog_or_keg=?";		
		$data = array(
					$this->id_status_baru, 
					$this->id_status_send, 
					$this->id_status_send, 
	...
	function import_from_dpa($id_skpd, $ta){
		...
		$query="SELECT 					
				...
				FROM tx_dpa_prog_keg_perubahan WHERE tx_dpa_prog_keg_perubahan.is_prog_or_keg=1 AND tahun=$ta AND tx_dpa_prog_keg_perubahan.id_skpd=?";
		...
		foreach ($cik_baru as $row) {			
			...
			$query="SELECT 
				...
				AND tx_dpa_prog_keg_perubahan.id_skpd=?";
