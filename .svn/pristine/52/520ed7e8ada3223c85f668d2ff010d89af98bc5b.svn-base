...
class M_rka extends CI_Model
{
	...
	function count_jendela_kontrol($id_skpd,$ta){
		$query = "SELECT
					...
					WHERE
						tahun = ?
					AND
						tx_rka_prog_keg.id_skpd=? AND is_prog_or_keg=?";
		$data = array(
					$this->id_status_baru,
					$this->id_status_send,
					$this->id_status_send,
	...
	function import_from_renja($id_skpd, $ta){
		...
		$query="SELECT
					...
					nominal,
					nominal_thndpn
				FROM t_renja_prog_keg WHERE t_renja_prog_keg.is_prog_or_keg=1 AND tahun=$ta AND t_renja_prog_keg.id_skpd=?";
		...
		foreach ($rka_baru as $row) {
			...
			$query = "INSERT INTO tx_rka_indikator_prog_keg(id_prog_keg, indikator, indikator_thndpn, satuan_target, satuan_target_thndpn, target, target_thndpn, id_indikator_renja) SELECT ?, indikator,indikator, satuan_target,satuan_target, target, target_thndpn, t_renja_indikator_prog_keg.id FROM t_renja_indikator_prog_keg WHERE id_prog_keg=?";
