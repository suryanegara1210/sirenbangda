...
<article class="module width_full" style="width:130%; margin-left: -15%;">
	<header>
		<h3>
			Tabel Rekapitulasi CIK Perubahan
		</h3>
	</header>
	<div class="module_content" style="overflow:auto">
	<div style='float:right'>
		<a href="<?php echo site_url('cik_perubahan/do_cetak_rekap_cik') ?>"><input style="margin: 3px 10px 0px 0px; float: right;" type="button" value="Cetak Rekapitulasi CIK Perubahan" /></a>
    </div><br><br>
		<table id="rekapitulasi_cik" class="table-common tablesorter" style="width:100%">
			<thead>
				<tr>
			...
			<tbody>
				<?php 
					foreach ($program as $row) {
						$result = $this->m_cik_perubahan->get_kegiatan_rekap_cik_4_cetak($row->id,$tahun_sekarang);
						$kegiatan = $result->result();
						$indikator_program = $this->m_cik_perubahan->get_indikator_prog_keg_rekap($row->id, FALSE, TRUE);
				...
				<tr>
					...
					<td style="border-bottom: 0;" rowspan="<?php echo $total_temp;?>">%CIK Perubahan</td>
					<td>
						<?php
				...
				?>
					...
					foreach ($kegiatan as $row) {						
							$indikator_kegiatan = $this->m_cik_perubahan->get_indikator_prog_keg_rekap($row->id, FALSE, TRUE);
