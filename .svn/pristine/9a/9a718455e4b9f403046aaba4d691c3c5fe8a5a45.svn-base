...
class Session
{
    ...
    /**
     * Refresh Session expire time if actively access site
     */
    public function sess_refresh()
    {
        $expire_time = time() + intval($this->sess_expiration);
        $_SESSION[$this->sess_namespace]['session_id'] = md5(microtime());
        $_SESSION[$this->sess_namespace]['expire_at'] = $expire_time;
        $this->store = $_SESSION[$this->sess_namespace];
    }
