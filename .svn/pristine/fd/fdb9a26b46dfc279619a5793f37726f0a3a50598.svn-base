	...
	<header>
		...
		<?php
			if (isset($isEdit) && $isEdit){
			    echo "Edit Data Pengedalian Belanja";
			} else{
			    echo "Input Data Pengedalian Belanja";
			}
		?>
		</h3>
 	</header>
 	<div class="module-content">
		<form method="post" name='input_kendali_belanja' id='input_kendali_belanja' action="<?php echo site_url('kendali_belanja/save_kendali_belanja')?>" enctype="multipart/form-data" >
		...
		<input type="hidden" name="id_kendali_belanja" value="<?php if(!empty($id_kendali_belanja)){echo $id_kendali_belanja;} ?>" />
            ...
            <div class="submit_link">  
    			...
    			<input type="button" value="Keluar" onclick="window.location.href='<?php echo site_url('kendali_belanja'); ?>'" />
                <input type="button" value="Batal" onclick="window.location='<?php $call_from = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '';
		          	if(!empty($call_from) && strpos($call_from, 'kendali_belanja/cru_kendali') == FALSE)
		          		echo $call_from;
		          	else 
		          		echo site_url('kendali_belanja/');
              	?>'"/>
  		    </div>
   		</form> 		
   	</div> 	
