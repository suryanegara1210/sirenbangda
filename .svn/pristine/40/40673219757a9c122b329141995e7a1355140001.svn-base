...
class Rka extends CI_Controller
{
	...
	public function __construct()
	{
        ...
        $this->load->model(array('m_rka','m_skpd','m_lov','m_urusan', 'm_bidang', 'm_program', 'm_kegiatan', 'm_template_cetak'));
	}
	...
	function save_program_rka(){
		...
		if (!empty($id)) {
			$result = $this->m_rka->edit_program_skpd($data, $id, $indikator, $id_indikator_program, $satuan_target, $target, $target_thndpn);
		}else{
			$result = $this->m_rka->add_program_skpd($data, $indikator, $satuan_target, $target, $target_thndpn);
		}
	...
	function save_kegiatan(){
		...
		if (!empty($id)) {
			$result = $this->m_rka->edit_kegiatan_skpd($data, $id, $indikator, $id_indikator_kegiatan, $satuan_target, $target, $target_thndpn);
		}else{
			$result = $this->m_rka->add_kegiatan_skpd($data, $indikator, $satuan_target, $target, $target_thndpn);
		}
