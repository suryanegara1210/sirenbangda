...
class Renstra extends CI_Controller
{
	...
	function save_program(){		
		...
		$clean = array('id_program', 'indikator_kinerja', 'id_indikator_program', 'satuan_target','kondisi_awal','target_1','target_2','target_3','target_4','target_5','target_kondisi_akhir');
		$data = $this->global_function->clean_array($data, $clean);

		if (!empty($id)) {
			$result = $this->m_renstra_trx->edit_program_skpd($data, $id, $indikator, $id_indikator_program, $satuan_target, $kondisi_awal, $target_1, $target_2, $target_3, $target_4, $target_5, $target_kondisi_akhir);
		}else{
			$result = $this->m_renstra_trx->add_program_skpd($data, $indikator, $satuan_target, $kondisi_awal, $target_1, $target_2, $target_3, $target_4, $target_5, $target_kondisi_akhir);
		}
	...
	function preview_kegiatan_renstra(){		
		...
		if (!empty($result)) {
			...
			$data['indikator_kegiatan'] = $this->m_renstra_trx->get_indikator_prog_keg($result->id, TRUE, TRUE);
	...
	function do_veri(){
		...
		$data['indikator'] = $this->m_renstra_trx->get_indikator_prog_keg($renstra->id, TRUE, TRUE);
		$data['program'] = FALSE;
