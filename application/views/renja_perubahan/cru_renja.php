...
<article class="module width_full">
	<header>
 	...
 	<div class="module-content">
		<form method="post" name='input_renja' id='input_renja' action="<?php echo site_url('renja_perubahan/save_renja')?>" enctype="multipart/form-data" >
		...
		<input type="hidden" name="id_status_renja" value="2"/>
            ...
            <div class="submit_link">  
    			...
    			<input type="button" value="Keluar" onclick="window.location.href='<?php echo site_url('renja_perubahan'); ?>'" />
                <input type="button" value="Batal" onclick="window.location='<?php $call_from = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '';
		          	if(!empty($call_from) && strpos($call_from, 'renja_perubahan/cru_renja') == FALSE)
		          		echo $call_from;
		          	else 
		          		echo site_url('renja_perubahan/');
              	?>'"/>
  		    </div>
   		</form> 		
   	</div> 	
