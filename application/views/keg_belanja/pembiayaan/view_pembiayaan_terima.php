<script type="text/javascript">
    ...
    $(document).ready(function(){
        dt = $("#pembiayaan_terima").DataTable({
          "processing": true,
            "serverSide": true,
            "aoColumnDefs": [
                  {
    ...
    function edit_pembiayaan_terima(id){
      window.location = '<?php echo $url_edit_data;?>/' + id;
    }

    function delete_pembiayaan_terima(id){
      if (confirm('Apakah anda yakin untuk menghapus data pembiayaan ini?')) {
        $.blockUI({
...
<article class="module widh_full">
  <header>
    <h3> TABEL DATA PENERIMAAN PEMBIAYAAN DAERAH</h3>
  </header>
  <div class="module_content"; style="overflow:auto">
    <div style='float:right'>
      <a href="<?php echo $url_add_data ?>"><input style="margin: 3px 10px 0 10px; float: right;" type="button" value="Tambah Data Penenrimaan Pembiayaan" /></a>
    </div><br>
  <table id="pembiayaan_terima" class="table-common tablesorter" style="width:100%">
    <thead>
      <tr>
