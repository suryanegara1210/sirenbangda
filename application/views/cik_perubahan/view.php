...
<script type="text/javascript">	
	...
	$(document).ready(function(){
		$(".tbh_program").click(function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("cik_perubahan/cru_program_skpd"); ?>',
		...
		$(".remove-program").click(function(){			
			if (confirm('Apakah anda yakin untuk menghapus data program ini?')) {				
				...
				$.ajax({
					type: "POST",
					url: '<?php echo site_url("cik_perubahan/delete_program"); ?>',
		...
		$("#program td.td-click").click(function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("cik_perubahan/get_kegiatan_skpd"); ?>',
				data: {id: idr},				
				success: function(msg){
					if (msg!="") {						
		...
		$(document).on("click", "#get_dpa", function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("cik_perubahan/get_dpa"); ?>',
				dataType: "json",
				success: function(msg){
	...
	function reload_jendela_kontrol(){
		$("#jendela_kontrol").load("<?php echo site_url('cik_perubahan/get_jendela_kontrol'); ?>");
	}
</script>
...
<?php if(!empty($jendela_kontrol->baru)){ ?>
			...
			<?php
				if (!empty($program)) {					
					...
					foreach ($program as $row) {
						$i++;
						$style='';
						$indikator_program = $this->m_cik_perubahan->get_indikator_prog_keg($row->id);
						$id= $row->id;
					if($row->id_dpa != NULL){
