	...
	<header>
		...
		<?php
			if (isset($isEdit) && $isEdit){
			    echo "Edit Data CIK Perubahan";
			} else{
			    echo "Input Data CIK Perubahan";
			}
		?>
		</h3>
 	</header>
 	<div class="module-content">
		<form method="post" name='input_cik' id='input_cik' action="<?php echo site_url('cik_perubahan/save_cik')?>" enctype="multipart/form-data" >
		...
		<input type="hidden" name="id_cik" value="<?php if(!empty($id_cik)){echo $id_cik;} ?>" />
            ...
            <div class="submit_link">  
                ...
                <input type="button" value="Batal" onclick="window.location='<?php $call_from = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '';
		          	if(!empty($call_from) && strpos($call_from, 'cik_perubahan/cru_cik') == FALSE)
		          		echo $call_from;
		          	else 
		          		echo site_url('cik/');
              	?>'"/>
