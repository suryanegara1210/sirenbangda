...
class M_dpa_perubahan extends CI_Model
{
	...
	function count_jendela_kontrol($id_skpd,$ta){
		$query = "SELECT
					...
					WHERE
						tahun = ?
					AND
						tx_dpa_prog_keg_perubahan.id_skpd=? AND is_prog_or_keg=?";
		$data = array(
					$this->id_status_baru,
					$this->id_status_send,
					$this->id_status_send,
	...
	function import_from_rka($id_skpd, $ta){
		...
		$query="SELECT
				...
				FROM tx_rka_prog_keg_perubahan WHERE tx_rka_prog_keg_perubahan.is_prog_or_keg=1 AND tahun=$ta AND tx_rka_prog_keg_perubahan.id_skpd=?";
		...
		foreach ($dpa_baru as $row) {
			...
			$query="SELECT
					...
					tx_rka_prog_keg_perubahan.id_skpd,
					? AS parent
				FROM tx_rka_prog_keg_perubahan WHERE tx_rka_prog_keg_perubahan.is_prog_or_keg=2 AND tahun=$ta AND tx_rka_prog_keg_perubahan.parent=? AND tx_rka_prog_keg_perubahan.id_skpd=?";
